...
MAX_MATCHES = int(os.environ.get("MAX_MATCHES", "6"))
...
USAGE = {"in": 0, "out": 0, "searches": 0, "calls": 0}
...
If there are none, return <json>[]</json>."""
    data = ask_json(prompt, max_uses=5)
    fx, seen = [], set()
    for f in data:
        key = (f["home"], f["away"])
        if key not in seen:
...
def pick_fixtures(fx, ledger, now):
    """Drop games already started or already analysed, then round-robin across leagues."""
    have = {(e["home"], e["away"], str(e["kickoff"])[:10]) for e in ledger}
    out = []
    for f in fx:
        try:
            k = dt.datetime.fromisoformat(f["kickoff_utc"].replace("Z", "+00:00"))
            if k.tzinfo is None:
                k = k.replace(tzinfo=dt.timezone.utc)
            if k <= now + dt.timedelta(minutes=20):
                continue                                   # started or about to start
        except Exception:
            pass
        if (f["home"], f["away"], str(f["kickoff_utc"])[:10]) in have:
            continue
        out.append(f)
    by = {}
    for f in out:
        by.setdefault(f["league"], []).append(f)
    rr = []
    while any(by.values()):
        for lg in list(by):
            if by[lg]:
                rr.append(by[lg].pop(0))
    return rr

# ---------------------------------------------------------------- analysis
def clean(obj):
    """Remove <cite ...> tags that web search leaves in text."""
    return json.loads(re.sub(r"</?cite[^>]*>", "", json.dumps(obj)))
...
def analyse(f, today):
    ...
    dossier = clean(ask_json(fill(SCOUT, **kw), max_uses=4))
    j = clean(ask_json(fill(COURT, **kw).replace("{DOSSIER}", json.dumps(dossier)), search=False))
...
const today=D.ledger.filter(e=>e.status=='pending').sort((a,b)=>a.kickoff<b.kickoff?-1:1);
h+='<h2>Upcoming / awaiting result - '+today.length+' matches</h2>';
...
def main():
    ...
    ledger = load()
    settle(ledger, now)
    fixtures = pick_fixtures(get_fixtures(today), ledger, now)
    print("new fixtures to analyse:", len(fixtures))
    for f in fixtures[:MAX_MATCHES]:
        try:
            ledger.append(analyse(f, today))
            print("done", f["league"], f["home"], f["away"])
        except Exception as ex:
            print("skip", f, ex)
